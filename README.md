...
첫 로그인 시 해당 유저에 대해 AccessToken과 RefreshToken을 발행하고 Front에 전달합니다. API 요청 시 header에 토큰을 함께 전달하고, Backend에서는 그 토큰이 유효한지만 검증합니다. 이를 통해 DB로의 I/O 없이 유저인증을 할 수 있었습니다.
...
이외의 오류들은 테스팅을 위해 Postman을 활용하며 단계별 검증을 통해 분석 후 해결할 수 있었습니다.
#### 4. 데이터 전송 경량화
